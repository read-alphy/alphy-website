...
export default function BasicCard({hasActiveSub,openPopover,setOpenPopover, currentUser, triggers, canceledAtPeriodEnd})
{
    const themePopover = {
        popover: {
          styles: {
            ...
            <div className={` h-[400px] ${currentUser=== null && "xl:h-[444px]"}`}>
                <ul role="list" className="space-y-5 my-7">
                    ...
                    <li className="flex space-x-3">
                        ...
                        <span className="text-base font-normal leading-tight text-gray-500 dark:text-zinc-300">10 hours of prioritized transcription credits per month</span>
                    </li>

                    <li className="flex space-x-3">
                    <svg aria-hidden="true" className="flex-shrink-0 w-5 h-5 text-greenColor" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Check icon</title><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path></svg>
                    {/* <span className="text-base font-normal leading-tight text-gray-500 dark:text-zinc-300">Optional credit top ups</span> */}
                    <span className="text-base font-normal leading-tight text-gray-500 dark:text-zinc-300">Create 3 Arcs</span>
                </li>
...
<ThemeProvider value={themePopover}>
      ...
      {/*               <li className="flex space-x-3">
            ...
            <Link className={`${hasActiveSub==="basic" && !canceledAtPeriodEnd ? "pointer-events-none" : ""}`} to={
                (currentUser && hasActiveSub==="free") ? `/plans/checkout` : (canceledAtPeriodEnd ? "https://billing.stripe.com/p/login/bIYdTS2Qs9CscfuaEE": "")}
>
                {currentUser ? <Button type="button" className={` bg-greenColor  transition duration-200 ease-in ${hasActiveSub==="basic" && !canceledAtPeriodEnd ? "pointer-events-none text-whiteLike" : ""} rounded-lg text-[16px] font-semibold ]px-5 py-3 inline-flex text-zinc-700 justify-center w-full text-center`} >{hasActiveSub==="basic" && !canceledAtPeriodEnd ? "Your Current Plan" : (canceledAtPeriodEnd ? "Renew Plan" : "Upgrade Plan")}</Button> : <div className={`h-[44px] ${currentUser?"block":"hidden"}`}></div>}
