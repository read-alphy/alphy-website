...
export default function ReadComponent({ data, transcript, summary, summaryArray, isLoading, handleClickTimestamp, handleDownload, handleAskAlphy, activeTab, setActiveTab, inputValue, setInputValue, selectionCall, setSelectionCall, buttonRef, inputRef, timestampChanger, languages, languagesWanted, language, errorMessage, contentSummaries, showYouTubeFrame, setShowYouTubeFrame, videoRef, canvasRef,  autoplay, timestamp, title, basicDataLoaded, setBasicDataLoaded, handleShowYouTubeFrame, contentRef, working, downloading, themePopover, language_codes, currentUser,QuestionAnswering,requestTranslation,tier}) {
  return (
								...
								<div className={`${data.summaries.length === 0 ? "hidden" : ""} grid-cols-2 w-full md:min-w-[500px]`}>
									...
									<div className={`col-span-2 ${data.source_type === "yt" && ""} drop-shadow-sm `}>
										{summary.key_qa !== undefined && summary.key_qa === null ? (
											<div id="q-and-a" className={`question-answering  md:min-h-[600px] border-b overflow-auto pt-10 pl-5 pr-5 pb-5 border border-zinc-100 dark:border-zinc-700   rounded-xl`}>
																							...
																							<div className="flex flex-row dark:text-zinc-300">
																								...
																								<div className={`${index !== 0 ? "hidden" : ""}   flex ml-auto justify-end flex-row justify-end `} >
																									...
																									>
																										...
																										<PopoverHandler>
																											<button id="popoverButtonDownload" data-popover-target="popoverHover" data-popover-trigger="hover" className={`${tier === "free" || tier === undefined ? "cursor-default dark:invert" : ""} mr-8 opacity-80 pt-4`} >
																												<button className={`${tier === "free" || tier === undefined ? "bg-indigo-200 text-white pointer-events-none" : ""} text-sm  font-averta-semibold bg-indigo-300 dark:bg-indigo-400 w-[180px] drop-shadow-sm rounded-lg p-2 text-white`}>
																									...
																									<Popover>
																										<PopoverHandler>
																											<button id="popoverButtonDownload" data-popover-target="popoverHover" data-popover-trigger="hover" className={`${tier === "free" || tier === undefined ? "cursor-default dark:invert" : ""} mr-8 opacity-80 pt-4`} >
																												...
																												<button className={`${tier === "free" || tier === undefined ? "bg-indigo-200 text-white pointer-events-none" : ""} text-sm bg-indigo-300 dark:bg-indigo-400 w-[180px] drop-shadow-sm rounded-lg p-2 text-white font-averta-semibold`}>
						...
						<div className="flex flex-col mb-20 mt-20 ">
							{(errorMessage === true || (languagesWanted.includes(language) === true) || languages.includes(language) || (summary !== undefined && summary.summary !== undefined && summary.summary !== null && summary.summary.length > 0) || (contentSummaries !== undefined && contentSummaries.length > 1 && (contentSummaries[0].lang === language || contentSummaries[1].lang === language)) || language === "en")
								...
								<p className="text-xl text-zinc-500 dark:text-zinc-200 font-averta-regular max-w-screen-md mx-auto p-3 text-center">
									...
									Seems like Alphy hasn't processed the content in {language_codes[language]} yet. {tier !== undefined && tier !== "free" ? <p className="font-averta-regular">Request Alphy to generate summary, key takeaways, and questions in {language_codes[language]} clicking <a onClick={requestTranslation} className="underline text-greenColor cursor-pointer">here</a>.</p>
