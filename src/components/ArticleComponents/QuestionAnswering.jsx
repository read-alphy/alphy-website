...
export default function QuestionAnswering(props) {
  ...
  useEffect(() => {
    setTimeout(() => {
      if (
        location.pathname.split('/')[2].split('&q=')[1] !== undefined &&
        clicked === false
      ) {
  ...
  const handleShareLink = () => {
    const encodedText = encodeURIComponent(
      props.inputValue.length > 0 ? props.inputValue : baseQuestion
    )

    const url = `${window.location.origin}${
      location.pathname.split('&q=')[0]
    }&q=${encodedText}`
  ...
  return (
    ...
    <div
      id="q-and-a"
      className={` md:min-h-[600px] lg:w-[700px] xl:w-[500px] 2xl:w-[500px] 3xl:w-full bg-white drop-shadow-sm dark:bg-mildDarkMode border-b overflow-auto mx-auto pt-10 pl-5 pr-5 pb-5 border border-zinc-100 dark:border-zinc-700   rounded-xl`}
      ref={QARef}
    >
