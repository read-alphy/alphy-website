...
function ArrayToQueryStrings(arrayOfObjects) {
    ...
    const queryStringsArray = arrayOfObjects.map(object => {return(`${object.title} \n\n ${object.summary} \n\n `)});
   
    return queryStringsArray.join('\n');
  }

function introPromptHandler(content,character,content_type,source_type,source_title,creator_name){

    if(Array.isArray(content)){content = ArrayToQueryStrings(content)}
    
    const characterPrompt = CharacterPrompts.find(prompt => prompt.name === character);

    let source_platform = "Twitter Space"
    if(source_type ==="yt"){source_platform = "YouTube"}
    if(source_type ==="sp"){source_platform = "Twitter Space"}
    if(source_type==="x"){source_platform = "Twitter"}
    if(source_type ==="tw"){ source_platform = "Twitch"}
...
const introPrompt = introPromptHandler(content_details.content, settings.character, content_details.content_type, content_details.source_type, content_details.source_title, content_details.creator_name)
