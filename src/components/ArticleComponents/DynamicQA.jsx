...
export default function DynamicQA({answerData, areaRefs, answer, data, handleClear, handleShareLink, handleCopyToClipboard, formatAnswer, singleSource, showSource, updateVariable, DataArrayIcon, handleShowAllSources, highlightIndex, handleLength}){
                    ...
                    <div>
                        <div>
                            ...
                            {answerData.sources.map((source, index) => (
                                <div ref={areaRefs.current[index]} className={`${(singleSource===true && showSource!== index+1) && "hidden"} border border-zinc-300 dark:border-zinc-600 rounded-lg p-5 drop-shadow-sm mb-5`} >
                                    ...
                                    {source.start !== null && source.start !== undefined && source.end ? (
                                        ...
                                        <p key={index}className="text-zinc-500 dark:text-zinc-400  font-normal mb-10 mt-6" dangerouslySetInnerHTML={{__html:handleLength(source.text)}}/>
