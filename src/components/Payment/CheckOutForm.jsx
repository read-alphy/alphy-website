...
export default function CheckOutForm({ clientSecret,currentUser }) {
  ...
  return (
    <form
      className="dark:text-whiteLike  pb-20"
      id="payment-form"
    ...
    >
      <LinkAuthenticationElement
        ...
        options={{
          defaultValues: { email: currentUser ? currentUser.email : '' },
        }}
      />
      {/*             <AddressElement
 ...
 */}
      ...
      <button
        className={`items-center w-full ${
          isLoading ? 'bg-green-200 pointer-events-none' : 'bg-greenColor'
        } ${
          paymentIntent.length > 0 ? 'pointer-events-none' : ''
        } text-whiteLike rounded-md mt-5 min-h-[40px] text-center`}
      ...
      >
        <span id="button-text" className="">
          {isLoading ? (
            ...
            <p className="flex flex-row items-center mx-auto text-center justify-center">
              <svg
                className=" mt-1 mr-1"
                width="20px"
                height="20px"
