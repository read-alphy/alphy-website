    ...
    return(
        ...
        <div className="col-span-2 flex justify-start  min-w-[300px]">
            ...
            placeholder="Set a title..."
							onChange={(event) => setArchipelagoTitle(event.target.value)} className="w-full text-l px-2 h-[50px] bg-white dark:bg-mildDarkMode border border-zinc-400 focus:border-green-400 focus:outline-none focus:ring-0 rounded-lg"/>
...
<div>
     ...
     {errorMessage && dataArchipelago.length===0 && <p className="mt-4 ml-2 mb-5 text-md text-red-500 dark:text-red-400 opacity-80">An Arc cannot be empty. Please add an item to continue.</p>}
                ...
                {dataArchipelago.length > 0
									?  dataArchipelago
										.map((item, index) => <FeedItem  key={index} item={item} mainFeedInput={inputValue} fromArchipelago={"archipelago"} dataArchipelago={dataArchipelago} setDataArchipelago={setDataArchipelago} sourceIDsArchipelago={sourceIDsArchipelago} setSourceIDsArchipelago={setSourceIDsArchipelago}/>
                                        )				
                                        :
                                       null
                }
