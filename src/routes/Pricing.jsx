...
<span className="text-base font-normal leading-tight text-gray-500 dark:text-zinc-300">Free Twitter Spaces transcription</span>
                        ...
                        {currentUser ? <button type="button" className="text-white bg-green-400 hover:bg-blue-800 transition duration-200 ease-in font-medium rounded-lg text-l px-5 py-2.5 inline-flex justify-center w-full text-center">{hasActiveSub ? "Your Current Plan" : "Upgrade Plan"}</button> : <div></div>}
                                    ...
                                    <a className={`${hasActiveSub && !canceledAtPeriodEnd ? "pointer-events-none" : ""}`} href={(currentUser && !hasActiveSub) && `/plans/checkout`}>
                                        {currentUser ? <button type="button" className={`text-white bg-green-400 transition duration-200 ease-in ${hasActiveSub & !canceledAtPeriodEnd ?  "pointer-events-none" : ""} font-medium rounded-lg text-l px-5 py-2.5 inline-flex justify-center w-full text-center`} >{hasActiveSub && !canceledAtPeriodEnd ? "Your Current Plan" : "Upgrade Plan"}</button> : <div></div>}
                                    </a>
